...
def place_order(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            order.save() #now pk is generated
            order.order_number = generate_order_number(order.id)
            order.vendors.add(*vendors_ids) # * Recursively add data to many to many field vendor
            order.save()
...
def payments(request):
    ...
    if request.headers.get('x-requested-with') == 'XMLHttpRequest' and request.method == 'POST':
        ...
        #MOVE THE CART ITEM TO ORDERFOOD MODEL
        ...
        #SEND ORDER RECEVIED MAIL TO RESTAURANT
        ...
        #Return back to ajax with success or failure
        response = {
...
def order_complete(request):
    ...
    try:
        ...
        return render(request, 'order/order_completion.html', context)
    except:
